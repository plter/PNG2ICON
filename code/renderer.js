...
class Renderer {
    ...
    addSelectImagesListeners() {
        ...
        $(".file-field").change(function () {
            if (this.files && this.files.length) {
                ...
                if (file && file.name && file.name.toLowerCase().endsWith(".png")) {
                    (async () => {
                        let url = await self.loadImage(file);
                        $(this.parentNode).find(".image").html(`<img style="display: block;" src="${url}" width="68" height="68">`);
                    })();
                } else {
                    alert("请选择png格式的图片");
                }
    ...
    btnConvertClickedHandler() {
        ...
        if (files.length) {
            let savedPath = electron.remote.dialog.showSaveDialog({title: "保存文件ico文件", defaultPath: "icon.ico"});
            if (savedPath) {
                (async () => {
